            ...
            <tbody>
                @foreach ($contacts as $contact)
                    <tr>
                        ...
                        <td class="no-wrap">{{ $contact->title }}</td>
                        <td>{{ $contact->message }}</td>
                        <td class="no-wrap">{{ $contact->created_at }}</td>
                        <td>
                            <a onclick="return confirm('Bạn có chắc chắn muốn xóa không')" class="waves-effect waves-light btn btn-delete btn-sm red darken-4" href="{{ action('Admin\ContactController@destroy', $contact->id) }}">@lang('admin.button.delete')</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
